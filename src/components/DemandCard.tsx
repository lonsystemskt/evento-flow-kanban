...
const DemandCard = ({ demand, onEdit, onDelete, onComplete }: DemandCardProps) => {
  ...
  return (
    <div className="min-w-[196px] max-w-[196px] min-h-[88px] bg-[#F6F7FB] rounded-lg border border-[rgba(0,0,0,0.05)] flex flex-col p-3">
      {/* Urgency Indicator */}
      ...
      <div className="flex-1 flex flex-col mb-2 text-left">
        <h4 className="font-semibold text-[#2E3A59] text-sm mb-1 line-clamp-2 leading-tight">
          {demand.title || 'Título não informado'}
        </h4>
        <p className="text-[#3B4D63] text-xs mb-1 line-clamp-2 leading-relaxed flex-1">
          {demand.subject || 'Assunto não informado'}
        </p>
        <p className="text-[#3B4D63]/50 text-xs font-medium">
